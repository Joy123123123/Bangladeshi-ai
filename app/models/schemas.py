...
class StudyRequest(BaseModel):
    topic: str = Field(..., min_length=1, max_length=500, description="Study topic or chapter")
    class_level: Optional[ClassLevel] = Field(None, description="NCTB class level")
    subject: Optional[SubjectCode] = Field(SubjectCode.GENERAL, description="Subject area")
    request_type: str = Field("summary", description="Type: summary | flashcards | quiz | mindmap")
    data_saver_mode: bool = Field(False, description="Enable low-data mode")
    preferred_model: AIModel = Field(AIModel.AUTO, description="Preferred AI model")


class AdmissionRequest(BaseModel):
    question: str = Field(..., min_length=1, max_length=2000, description="Admission exam question or topic")
    exam_type: ExamType = Field(ExamType.BUET, description="Target admission exam")
    subject: Optional[SubjectCode] = Field(None, description="Subject area")
    request_type: str = Field("solve", description="Type: solve | tips | pattern | mock")
    data_saver_mode: bool = Field(False, description="Enable low-data mode")
    preferred_model: AIModel = Field(AIModel.AUTO, description="Preferred AI model")
