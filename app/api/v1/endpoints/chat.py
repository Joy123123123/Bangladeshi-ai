"""
FastAPI streaming chat endpoint with NCTB context injection.
...
from app.services.ai_routing_service import ai_router
...
async def _event_stream(
    ...
    """
    try:
        # 1. Parse & normalise Banglish input
        parse_result = await banglish_parser.full_parse(request.message)
        ...
        async for text_chunk in ai_router.route(
            ...
            # Emit when we have enough characters (respects data_saver chunk size)
        ...
        error_payload = json.dumps({"content": "", "done": True, "error": str(exc)}, ensure_ascii=False)
...
async def chat_stream(request: ChatRequest) -> StreamingResponse:
    ...
    logger.info(
        "Chat request | session=%s | class=%s | subject=%s | model=%s",
        session_id,
        request.class_level,
        request.subject,
